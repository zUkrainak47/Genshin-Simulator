# from artifact_simulator import transmute, sets, substats, artifact_types, type_to_main_stats, Artifact
# import customtkinter as ctk
# from tkinter import messagebox
#
#
# # Function to handle the Transmute button click
# # def transmute(preset):
# #     messagebox.showinfo("Transmute", f"Transmuting with preset: {preset}")
#
#
# def update_button_state():
#     # Count the number of selected sub stats
#     selected_substats = sum([var.get() for var in substat_vars])
#
#     # Enable or disable Transmute button based on number of selected substats
...
#
# def on_transmute():
#     # Retrieve values for the preset
...
#
#     # Call transmute with the preset
#     art, _ = transmute(preset)
#     art.print_stats()
#
#
# def update_main_stats(*args):
#     # Update the main stats dropdown based on selected artifact type
#     artifact_type = artifact_type_var.get()
#     new_main_stats = type_to_main_stats[artifact_type]
#     main_stat_menu.configure(values=new_main_stats)
#     main_stat_var.set(new_main_stats[0])  # Set the first item as default
#
#
# # Sample data for dropdowns
# main_stats = ["HP%", "ATK%", "DEF%", "Elemental Mastery", "Energy Recharge"]
#
...
# artifact_set_menu = ctk.CTkOptionMenu(app, variable=artifact_set_var, values=sets)
# artifact_set_menu.grid(row=0, column=1, padx=10, pady=10)
#
# # Artifact Type Dropdown
# artifact_type_var = ctk.StringVar(value=artifact_types[0])
# artifact_type_var.trace_add("write", update_main_stats)  # Update main stats when artifact type changes
...
from artifact_simulator import transmute, sets, substats, artifact_types, type_to_main_stats, Artifact, set_to_image
...
def open_artifact_set_dropdown():
    ...
    for artifact_set, image in set_to_image.items():
        ...
        btn = ctk.CTkButton(dropdown_window, text=artifact_set, image=btn_image, compound="left",
